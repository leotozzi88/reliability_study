function compute_gmet_sherlock(atlas, threshold, connmatsfolder, outputfolder)
...
excludemotion={'102008' '104820' '124220' '140824' '147030' '151223' '151526' '153833' '159138' '161327' '161832' '163331' '171532' '175439' '205220' '223929' '233326' '298051' '311320' '352132' '366446' '392447' '453542' '562345' '748662' '833249' '947668'};

subs=allsubs(~ismember(allsubs, excludemotion));

atlases={atlas};
 
% Generating measures of interest

for atlas=1:1 %todo inizializza le variabili per avere atlanti diversi
    
    disp(strcat('Starting atlas: ', atlases{atlas}))
    
    % Loop through sparsities
    ...
    for thresh=threshold
        ...
        for sub=1:length(subs)
            ...
            disp(strcat('Processing subject: ', subs{sub}))
            
            % Load matrices
            
	   cmat1_unt=csvread(strcat(connmatsfolder, '/', atlases{atlas}, '/', subs{sub}, '_rfMRI_REST1_concat_', atlases{atlas}, '.csv'));
           cmat2_unt=csvread(strcat(connmatsfolder, '/', atlases{atlas}, '/', subs{sub}, '_rfMRI_REST2_concat_', atlases{atlas}, '.csv'));
            
            % Sparsify matrices
            
            cmat1_tw=threshold_proportional(cmat1_unt, thresh);
            cmat2_tw=threshold_proportional(cmat2_unt, thresh);
            
            % Binarise
            
            cmat1_tu=threshold_proportional(cmat1_tw, thresh)>0;
            cmat2_tu=threshold_proportional(cmat2_tw, thresh)>0;
            
            % Unpack matrix
            
            mask=logical(triu(ones(size(cmat1_unt)),1));
            edgw1(sub,:)=cmat1_tw(mask)';
            edgw2(sub,:)=cmat2_tw(mask)';
            
            edgu1(sub,:)=cmat1_tu(mask)';
            edgu2(sub,:)=cmat2_tu(mask)';
            
            % Compute degree
            
            deg1(sub, :)=degrees_und(cmat1_tw);
            deg2(sub, :)=degrees_und(cmat2_tw);
            
            % Compute strength
            
            strw1(sub, :)=strengths_und(cmat1_tw);
            strw2(sub, :)=strengths_und(cmat2_tw);
            
            stru1(sub, :)=strengths_und(cmat1_tu);
            stru2(sub, :)=strengths_und(cmat2_tu);
            
            % Clustering coefficient
            
            [C_pos1,C_neg1,Ctot_pos1,Ctot_neg1] = clustering_coef_wu_sign(cmat1_tw);
            cluposw1(sub, :)=C_pos1';
            clunegw1(sub, :)=C_neg1';
            clutotposw1(sub,1)=Ctot_pos1;
            clutotnegw1(sub,1)=Ctot_neg1;
            
            [C_pos2,C_neg2,Ctot_pos2,Ctot_neg2] = clustering_coef_wu_sign(cmat2_tw);
            cluposw2(sub, :)=C_pos2';
            clunegw2(sub, :)=C_neg2';
            clutotposw2(sub,1)=Ctot_pos2;
            clutotnegw2(sub,1)=Ctot_neg2;
            
            cluu1(sub,:)=clustering_coef_bu(cmat1_tu)';
            cluu2(sub,:)=clustering_coef_bu(cmat2_tu)';
            
            % Global efficiency
            geffw1(sub, 1)=efficiency_wei(cmat1_tw);
            geffw2(sub, 1)=efficiency_wei(cmat2_tw);
            
            geffu1(sub, 1)=efficiency_bin(cmat1_tu);
            geffu2(sub, 1)=efficiency_bin(cmat2_tu);
            
            % Local efficiency
            leffw1(sub, :)=efficiency_wei(cmat1_tw,1)';
            leffw2(sub, :)=efficiency_wei(cmat2_tw,1)';
            
            leffu1(sub, :)=efficiency_bin(cmat1_tu,1);
            leffu2(sub, :)=efficiency_bin(cmat2_tu,1);
            
        end
        
        % Save measures of interest
        
	disp(strcat('Saving outputs in', {' '}, outputfolder)) 
        cd(strcat(outputfolder, '/', atlases{atlas}))
        
        csvwrite(strcat('edgw1', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), edgw1)
        csvwrite(strcat('edgw2', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), edgw2)
        csvwrite(strcat('edgu1', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), edgu1)
        csvwrite(strcat('edgu2', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), edgu2)
        csvwrite(strcat('deg1', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), deg1)
        csvwrite(strcat('deg2', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), deg2)
        csvwrite(strcat('strw1', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), strw1)
        csvwrite(strcat('strw2', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), strw2)
        csvwrite(strcat('stru1', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), stru1)
        csvwrite(strcat('stru2', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), stru2)
        csvwrite(strcat('cluposw1', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), cluposw1)
        csvwrite(strcat('clunegw1', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), clunegw1)
        csvwrite(strcat('clutotposw1', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), clutotposw1)
        csvwrite(strcat('clutotnegw1', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), clutotnegw1)
        csvwrite(strcat('cluposw2', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), cluposw2)
        csvwrite(strcat('clunegw2', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), clunegw2)
        csvwrite(strcat('clutotposw2', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), clutotposw2)
        csvwrite(strcat('clutotnegw2', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), clutotnegw2)
        csvwrite(strcat('cluu1', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), cluu1)
        csvwrite(strcat('cluu2', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), cluu2)
        csvwrite(strcat('geffw1', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), geffw1)
        csvwrite(strcat('geffw2', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), geffw2)
        csvwrite(strcat('geffu1', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), geffu1)
        csvwrite(strcat('geffu2', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), geffu2)
        csvwrite(strcat('leffw1', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), leffw1)
        csvwrite(strcat('leffw2', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), leffw2)
        csvwrite(strcat('leffu1', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), leffu1)
        csvwrite(strcat('leffu2', '_', atlases{atlas}, '_', num2str(thresh), '.csv'), leffu2)
