...
matpath='/Users/leonardotozzi/Desktop/Repeatability_study/Connmats'; % path to the matrices
...
% For each atlas and subject, load matrix, threshold and unpack
...
for atlas=1:length(atlases)
    ...
    % saving edges as a sub x edge x threshold 3d matrix using '-v7.3'
    % option for large files
    
    %     save(strcat(edges_outputpath, '/', atlases{atlas}, '_perc_edg1'), '-v7.3', 'perc_edg1')
    %     save(strcat(edges_outputpath, '/', atlases{atlas}, '_perc_edg2'),'-v7.3', 'perc_edg2')
    %     save(strcat(edges_outputpath, '/', atlases{atlas}, '_abs_edg1'), '-v7.3', 'abs_edg1')
    %     save(strcat(edges_outputpath, '/', atlases{atlas}, '_abs_edg2'), '-v7.3', 'abs_edg2')
    %
    %
    %     % Now calculate ICC and jaccard index for percent thresholded matrices
    %
    %     disp('Calculating ICC for percent thresholded matrices')
    %
    %     icc_noGSR=nan(1, feats);
    %     allji=nan(1, feats);
    %     icc_noGSR_thr=nan(length(thresholds), feats);
    %     allji_noGSR_thr=nan(length(thresholds), feats);
    %     icclevelsnoGSR=nan(1, 7);
    %     JI_icclevelsnoGSR=nan(1, 7);
    %
    %     for thresh=1:length(thresholds)
    %
    %         % for each threshold compute ICC and jaccard index for each feature
    %
    %         for feat=1:size(perc_edg1, 2)
    %
    %             % ICC
    %
    %             data=[ perc_edg1(:,feat, thresh) perc_edg2(:,feat, thresh) ];
    ...
    %                 allji(1,feat)=1;
    %             else
    %                 allji(1,feat) = sum(data(:, 1)>0 & data(:, 2)>0)/(sum(data(:,1)>0) + sum(data(:,2)>0));
    %             end
    %
    %         end
    %
    %         % save egewise ICC and JI
    %
    %         icc_noGSR_thr(thresh,:)=icc_noGSR;
    %         allji_noGSR_thr(thresh,:)=allji;
    %
    %         % levels and median of ICC
    %
    %         ICC_poor_nogsr=sum(icc_noGSR<0.40, 2)/size(icc_noGSR, 2);
    %         ICC_fair_nogsr=sum((0.40<=icc_noGSR) & (icc_noGSR<0.60), 2)/size(icc_noGSR, 2);
    %         ICC_good_nogsr=sum((0.60<=icc_noGSR) & (icc_noGSR<0.75), 2)/size(icc_noGSR, 2);
    %         ICC_excellent_nogsr=sum(icc_noGSR>=0.75, 2)/size(icc_noGSR, 2);
    %         ICC_medianicc_nogsr=median(icc_noGSR, 2);
    %         ICC_minicc_gsr=min(icc_noGSR, [], 2);
    %         ICC_maxicc_gsr=max(icc_noGSR, [], 2);
    %
    %         icclevelsnoGSR(thresh, :)=[ICC_poor_nogsr ICC_fair_nogsr ICC_good_nogsr ICC_excellent_nogsr ICC_medianicc_nogsr ICC_minicc_gsr ICC_maxicc_gsr];
    %
    %         % levels and median of jaccard
    %
    %         JI_poor_nogsr=sum(allji<0.40, 2)/size(allji, 2);
    %         JI_fair_nogsr=sum((0.40<=allji) & (allji<0.60), 2)/size(allji, 2);
    %         JI_good_nogsr=sum((0.60<=allji) & (allji<0.75), 2)/size(allji, 2);
    %         JI_excellent_nogsr=sum(allji>=0.75, 2)/size(allji, 2);
    %         JI_median_nogsr=median(allji, 2);
    %         JI_min_gsr=min(icc_noGSR, [], 2);
    %         JI_max_gsr=max(icc_noGSR, [], 2);
    %
    %         JI_icclevelsnoGSR(thresh, :)=[JI_poor_nogsr JI_fair_nogsr JI_good_nogsr JI_excellent_nogsr JI_median_nogsr JI_min_gsr JI_max_gsr];
    %
    %     end
    %
    %     % saving results as a threshold x feature matrix
    %
    %     csvwrite(strcat(ICC_outputpath, '/ICC_alledg_perc_thresh', '_', atlases{atlas}, '.csv'), icc_noGSR_thr)
    %     csvwrite(strcat(ICC_outputpath, '/JI_alledg_perc_threshthr', '_', atlases{atlas}, '.csv'), allji_noGSR_thr)
    %     csvwrite(strcat(ICC_outputpath, '/ICC_lvl_perc_thresh', '_', atlases{atlas}, '.csv'), icclevelsnoGSR)
    %     csvwrite(strcat(ICC_outputpath, '/JI_lvl_perc_threshthr', '_', atlases{atlas}, '.csv'), JI_icclevelsnoGSR)
    %
    %
    %     % Now calculate ICC and jaccard index for absolute thresholded matrices
    %
    %     disp('Calculating ICC for absolute thresholded matrices')
    %
    %     icc_noGSR=nan(1, feats);
    %     allji=nan(1, feats);
    %     icc_noGSR_thr=nan(length(thresholds), feats);
    ...
    %             % ICC
    %
    %             data=[ abs_edg1(:,feat, thresh) abs_edg2(:,feat, thresh) ];
    %
    %             % Keep in mind that if all edges are zero, NaNs are produced
    %             icc_noGSR(1,feat) = ICC(data, 'C-1');
    %
    %             % jaccard
    %             if (sum(data(:,1)>0) + sum(data(:,2)>0))==0
    ...
    %     % saving results as a threshold x feature matrix
    %
    %     csvwrite(strcat(ICC_outputpath, '/ICC_alledg_abs_thresh', '_', atlases{atlas}, '.csv'), icc_noGSR_thr)
    %     csvwrite(strcat(ICC_outputpath, '/JI_alledg_abs_threshthr', '_', atlases{atlas}, '.csv'), allji_noGSR_thr)
    %     csvwrite(strcat(ICC_outputpath, '/ICC_lvl_abs_thresh', '_', atlases{atlas}, '.csv'), icclevelsnoGSR)
    %     csvwrite(strcat(ICC_outputpath, '/JI_lvl_abs_threshthr', '_', atlases{atlas}, '.csv'), JI_icclevelsnoGSR)
    %
    
end

% Instead, compute how many times an edge is consistently kept and how many
% consistently removed

%
% for atlas=1:length(atlases)
%
%     disp(strcat('Computing consistency of edges kept/discarded for atlas:' , atlases{atlas}))
%
%     % preallocating the 3d matrices
%
%     if strcmp(atlases{atlas}, 'BNT')
...
%     end
%
%     abs_conskept=nan(length(thresholds), feats);
%     perc_conskept=nan(length(thresholds), feats);
%     abs_consdisc=nan(length(thresholds), feats);
%     perc_consdisc=nan(length(thresholds), feats);
%
%
%     load(strcat(edges_outputpath, '/', atlases{atlas}, '_perc_edg1'))
%     load(strcat(edges_outputpath, '/', atlases{atlas}, '_perc_edg2'))
%     load(strcat(edges_outputpath, '/', atlases{atlas}, '_abs_edg1'))
%     load(strcat(edges_outputpath, '/', atlases{atlas}, '_abs_edg2'))
%
%     for thresh=1:length(thresholds)
%
%         for feat=1:size(perc_edg1, 2)
%
%             % Ratio of times in which the same edge is kept at both sessions
%             abs_conskept(thresh, feat)=sum((abs_edg1(:,feat, thresh)>0) & (abs_edg2(:,feat, thresh)>0))/size(abs_edg1, 1);
%             perc_conskept(thresh, feat)=sum((perc_edg1(:,feat, thresh)>0) & (perc_edg2(:,feat, thresh)>0))/size(abs_edg1, 1);
%
%             % Ratio of times in which the same edge is discarded at both sessions
%             abs_consdisc(thresh, feat)=sum((abs_edg1(:,feat, thresh)==0) & (abs_edg2(:,feat, thresh)==0))/size(abs_edg1, 1);;
%             perc_consdisc(thresh, feat)=sum((perc_edg1(:,feat, thresh)==0) & (perc_edg2(:,feat, thresh)==0))/size(abs_edg1, 1);;
%
%         end
%
%     end
%
%     csvwrite(strcat(ICC_outputpath, '/abs_conskept', '_', atlases{atlas}, '.csv'), abs_conskept)
%     csvwrite(strcat(ICC_outputpath, '/perc_conskept', '_', atlases{atlas}, '.csv'), perc_conskept)
%     csvwrite(strcat(ICC_outputpath, '/abs_consdisc', '_', atlases{atlas}, '.csv'), abs_consdisc)
%     csvwrite(strcat(ICC_outputpath, '/perc_consdisc', '_', atlases{atlas}, '.csv'), perc_consdisc)
%
%     % Levels
%
%     abs_conskept_poor=sum(abs_conskept<0.40, 2)/size(abs_conskept, 2);
%     abs_conskept_fair=sum((0.40<=abs_conskept) & (abs_conskept<0.60), 2)/size(abs_conskept, 2);
%     abs_conskept_good=sum((0.60<=abs_conskept) & (abs_conskept<0.75), 2)/size(abs_conskept, 2);
%     abs_conskept_excellent=sum(abs_conskept>=0.75, 2)/size(abs_conskept, 2);
%     abs_conskept_median=median(abs_conskept, 2);
%     abs_conskept_min=min(abs_conskept, [], 2);
%     abs_conskept_max=max(abs_conskept, [], 2);
%     abs_conskept_lvl=[abs_conskept_poor abs_conskept_fair abs_conskept_good abs_conskept_excellent abs_conskept_median abs_conskept_min abs_conskept_max];
%
%     perc_conskept_poor=sum(perc_conskept<0.40, 2)/size(perc_conskept, 2);
%     perc_conskept_fair=sum((0.40<=perc_conskept) & (perc_conskept<0.60), 2)/size(perc_conskept, 2);
%     perc_conskept_good=sum((0.60<=perc_conskept) & (perc_conskept<0.75), 2)/size(perc_conskept, 2);
%     perc_conskept_excellent=sum(perc_conskept>=0.75, 2)/size(perc_conskept, 2);
%     perc_conskept_median=median(perc_conskept, 2);
%     perc_conskept_min=min(perc_conskept, [], 2);
%     perc_conskept_max=max(perc_conskept, [], 2);
%     perc_conskept_lvl=[perc_conskept_poor perc_conskept_fair perc_conskept_good perc_conskept_excellent perc_conskept_median perc_conskept_min perc_conskept_max];
%
%     abs_consdisc_poor=sum(abs_consdisc<0.40, 2)/size(abs_consdisc, 2);
%     abs_consdisc_fair=sum((0.40<=abs_consdisc) & (abs_consdisc<0.60), 2)/size(abs_consdisc, 2);
%     abs_consdisc_good=sum((0.60<=abs_consdisc) & (abs_consdisc<0.75), 2)/size(abs_consdisc, 2);
%     abs_consdisc_excellent=sum(abs_consdisc>=0.75, 2)/size(abs_consdisc, 2);
%     abs_consdisc_median=median(abs_consdisc, 2);
%     abs_consdisc_min=min(abs_consdisc, [], 2);
%     abs_consdisc_max=max(abs_consdisc, [], 2);
%     abs_consdisc_lvl=[abs_consdisc_poor abs_consdisc_fair abs_consdisc_good abs_consdisc_excellent abs_consdisc_median abs_consdisc_min abs_consdisc_max];
%
%     perc_consdisc_poor=sum(perc_consdisc<0.40, 2)/size(perc_consdisc, 2);
%     perc_consdisc_fair=sum((0.40<=perc_consdisc) & (perc_consdisc<0.60), 2)/size(perc_consdisc, 2);
%     perc_consdisc_good=sum((0.60<=perc_consdisc) & (perc_consdisc<0.75), 2)/size(perc_consdisc, 2);
%     perc_consdisc_excellent=sum(perc_consdisc>=0.75, 2)/size(perc_consdisc, 2);
%     perc_consdisc_median=median(perc_consdisc, 2);
%     perc_consdisc_min=min(perc_consdisc, [], 2);
%     perc_consdisc_max=max(perc_consdisc, [], 2);
%     perc_consdisc_lvl=[perc_consdisc_poor perc_consdisc_fair perc_consdisc_good perc_consdisc_excellent perc_consdisc_median perc_consdisc_min perc_consdisc_max];
%
%     csvwrite(strcat(ICC_outputpath, '/abs_conskept_lvl', '_', atlases{atlas}, '.csv'), abs_conskept_lvl)
%     csvwrite(strcat(ICC_outputpath, '/perc_conskept_lvl', '_', atlases{atlas}, '.csv'), perc_conskept_lvl)
%     csvwrite(strcat(ICC_outputpath, '/abs_consdisc_lvl', '_', atlases{atlas}, '.csv'), abs_consdisc_lvl)
%     csvwrite(strcat(ICC_outputpath, '/perc_consdisc_lvl', '_', atlases{atlas}, '.csv'), perc_consdisc_lvl)
%
% end
...
for atlas=1:length(atlases)
    ...
    for thresh=1:length(thresholds)
        ...
        % Now calculate how many times an edge is consistently kept out of
        % the times it is ever kept
...
for atlas=1:length(atlases)
    ...
    abs_ratiokept_lvl=csvread(strcat(ICC_outputpath, '/abs_ratiokept_lvl', '_', atlases{atlas}, '.csv'));
    perc_ratiokept_lvl=csvread(strcat(ICC_outputpath, '/perc_ratiokept_lvl', '_', atlases{atlas}, '.csv'));
    abs_icc_onlycons_lvl=csvread(strcat(ICC_outputpath, '/abs_icc_onlycons_lvl', '_', atlases{atlas}, '.csv'));
    perc_icc_onlycons_lvl=csvread(strcat(ICC_outputpath, '/perc_icc_onlycons_lvl', '_', atlases{atlas}, '.csv'));

    figure()
plot(thresholds(1:18), abs_icc_onlycons_lvl(1:18,1:5), 'LineWidth',2)
legend(names, 'Location','best')
title(strcat('ICC of consistent edges for absolute threshold', {' '}, atlases{atlas}))

    figure()
plot(thresholds(1:18), perc_icc_onlycons_lvl(1:18,1:5), 'LineWidth',2)
legend(names, 'Location','best')
title(strcat('ICC of consistent edges for relative threshold', {' '}, atlases{atlas}))

    figure()
plot(thresholds(1:18), abs_ratiokept_lvl(1:18,1:5), 'LineWidth',2)
legend(names, 'Location','best')
title(strcat('Ratio of consistent edges for absolute threshold', {' '}, atlases{atlas}))

    figure()
plot(thresholds(1:18), perc_ratiokept_lvl(1:18,1:5), 'LineWidth',2)
legend(names, 'Location','best')
title(strcat('Ratio of consistent edges for relative threshold', {' '}, atlases{atlas}))
